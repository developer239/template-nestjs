...
VITEST_OVERRIDES := --poolOptions.threads.singleThread
...
test:
	NODE_ENV=test npx vitest ${VITEST_OVERRIDES}

test-watch:
	NODE_ENV=test npx vitest --watch ${VITEST_OVERRIDES}

coverage:
	NODE_ENV=test npx vitest --coverage ${VITEST_OVERRIDES}
